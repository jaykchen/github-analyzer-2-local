            ...
            .send(&mut writer)
    {
        Ok(res) => {
            ...
            match serde_json::from_slice::<SearchResponse>(&writer) {
                Err(_e) => {
                    println!("Error parsing SearchResponse: {:?}", _e);
                    None
                }
                Ok(search_response) => {
...
pub async fn get_repo_info(about_repo: &str) -> Option<String> {
    ...
    let community_profile_url =
        format!("https://api.github.com/repos/{}/community/profile", about_repo);
    ...
    match octocrab.get::<CommunityProfile, _, ()>(&community_profile_url, None::<&()>).await {
        Ok(profile) => {
            ...
            date = profile.updated_at.as_ref().unwrap_or(&Utc::now()).date_naive();
        }
    ...
    match get_readme_owner_repo(about_repo).await {
        Some(content) => {
            ...
            match analyze_readme(&content).await {
                ...
                None => println!("Error parsing README.md: {}", about_repo),
            }
        }
        None => println!("Error fetching README.md: {}", about_repo),
    }
...
pub async fn get_repo_overview_by_scraper(about_repo: &str) -> Option<String> {
    ...
    match chat_inner(sys_prompt, usr_prompt, 700).await {
        Ok(r) => {
            return Some(r);
        }
        Err(_e) => {
            println!("Error summarizing meta data: {}", _e);
            return None;
        }
    }
}
...
"#
    ...
    // let usr_prompt_2 = &format!(
    //     r#"Analyze the key technical contributions made by {target_str} this week. Consider the following points and express the information in JSON format with direct textual descriptions as string values for each field. If no information is available for a point, leave it blank as an empty string.
    //     - Highlight impactful contributions and their interconnections as a concise summary.
    //     - Explain alignment with the project's goals in a clear statement.
    //     - Identify recurring patterns or trends in the contributions.
    //     - Discuss synergy between individual and collective advancement in a single sentence.
    //     - Comment objectively on the significance of each contribution in a succinct manner.
    //     Please reply with valid, iterable RFC8259 compliant JSON, ensuring that each value is a plain string and not a nested structure. Use the following template for your response:
    //     ```
    //     {{
    //       "impactful": "A concise summary of impactful contributions and their interconnections.",
    //       "alignment": "A clear statement explaining alignment with the project's goals.",
    //       "patterns": "A description of recurring patterns or trends in the contributions.",
    //       "synergy": "A sentence discussing the synergy between individual and collective advancement.",
    //       "significance": "A succinct comment on the significance of each contribution."
    //     }}
    //     ```"#,
    // );
    chain_of_chat(
